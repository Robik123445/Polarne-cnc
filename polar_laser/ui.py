...
class WorkspaceView(QGraphicsView):
    """2D workspace view with zooming and panning for path preview."""
    ...
    def wheelEvent(self, event):
        """Zoom in/out around mouse cursor with wheel input."""
...
class MainWindow(QMainWindow):
    ...
    def recalculate(self) -> None:
        ...
        try:
            ...
            self.gcode_preview.setPlainText(gcode[:15000])
    ...
    def _render_scene(self) -> None:
        ...
        r_pen.setWidthF(0)
        self.scene.addEllipse(self.pivot_x - self.machine.r_max_mm, self.pivot_y - self.machine.r_max_mm,
                              2 * self.machine.r_max_mm, 2 * self.machine.r_max_mm, r_pen)

        paths = self.processed.xy_paths if self.processed else self.xy_paths
        path_pen = QPen(Qt.black)
        path_pen.setWidthF(0)

        for idx, poly in enumerate(paths):
        ...
        if self.processed:
            ...
            for path_idx, seg_idx in self.processed.out_of_range_segments:
                ...
                a = pts[seg_idx]
                b = pts[seg_idx + 1]
                self.scene.addLine(a[0] + self.pivot_x, a[1] + self.pivot_y, b[0] + self.pivot_x, b[1] + self.pivot_y, bad_pen)
